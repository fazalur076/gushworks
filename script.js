document.addEventListener('DOMContentLoaded', () => {
    ...
    // ───── MANUFACTURING TABS ─────
    const mfgTabs = document.querySelectorAll('.mfg-tab-btn');
    const mfgBody = document.getElementById('mfg-tab-body');
    const mfgImg = document.getElementById('mfg-display-img');
    const mfgMPrev = document.getElementById('mfg-m-prev');
    const mfgMNext = document.getElementById('mfg-m-next');

    if (mfgTabs.length > 0 && mfgBody && mfgImg) {
        const mfgData = {
            raw: {
                stepLabel: 'Step 1/8: Raw Material',
            ...
            extrusion: {
                stepLabel: 'Step 2/8: Extrusion',
                title: 'Precision Extrusion',
                text: 'Advanced extrusion lines with ultrasonic monitoring ensure uniform wall thickness and roundness.',
                bullets: ['Uniform wall thickness', 'NABL standard extrusion'],
                img: 'https://images.unsplash.com/photo-1567789884554-0b844b597180?w=700&q=80'
            },
            cooling: {
                stepLabel: 'Step 3/8: Cooling',
                title: 'Efficient Cooling Process',
                text: 'Multi-stage vacuum cooling tanks stabilize the pipe dimensions and lock in material properties.',
                bullets: ['Multi-stage cooling', 'Dimensional stability'],
                img: 'https://images.unsplash.com/photo-1544724569-5f546fd6f2b5?w=700&q=80'
            },
            sizing: {
                stepLabel: 'Step 4/8: Sizing',
                title: 'Vacuum Sizing & Calibration',
                text: 'Ensures the pipe meets strict tolerance levels for outer diameter and wall thickness.',
                bullets: ['Precise calibration', 'Strict tolerance control'],
                img: 'https://images.unsplash.com/photo-1541888946425-d81bb19480c5?w=700&q=80'
            },
            qc: {
                stepLabel: 'Step 5/8: Quality Control',
                title: 'Rigorous Quality Control',
                text: 'In-line and laboratory testing for hydrostatic pressure, tensile strength, and material density.',
                bullets: ['100% in-line inspection', 'NABL accredited lab testing'],
                img: 'https://images.unsplash.com/photo-1565008576549-57569a49371d?w=700&q=80'
            },
            marking: {
                stepLabel: 'Step 6/8: Marking',
                title: 'Standardized Marking',
                text: 'Automated inkjet marking for traceability, including size, pressure rating, and manufacturing date.',
                bullets: ['Full traceability', 'Inkjet marking'],
                img: 'https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=700&q=80'
            },
            cutting: {
                stepLabel: 'Step 7/8: Cutting',
                title: 'Precision Cutting',
                text: 'Planetary cutters provide clean, squared-off ends for perfect jointing in the field.',
                bullets: ['Squared-off ends', 'Custom length cutting'],
                img: 'https://images.unsplash.com/photo-1567789884554-0b844b597180?w=700&q=80'
            },
            packaging: {
                stepLabel: 'Step 8/8: Packaging',
                title: 'Secure Packaging & Dispatch',
                text: 'Coils and straight lengths are securely bundled and capped for safe transport and storage.',
                bullets: ['Coil bundling', 'Protective end caps'],
                img: 'https://images.unsplash.com/photo-1500937386664-56d1dfef3854?w=700&q=80'
            }
        };

        const tabKeys = Object.keys(mfgData);
        let currentIndex = 0;

        const updateMfgContent = (index) => {
            currentIndex = index;
            const key = tabKeys[currentIndex];
            const data = mfgData[key];
            ...
            mfgBody.style.opacity = '0';
            mfgImg.style.opacity = '0';

            setTimeout(() => {
                if (data) {
                    mfgBody.innerHTML = `
                        <div class="mfg-step-badge">${data.stepLabel}</div>
                        <h3>${data.title}</h3>
                        <p>${data.text}</p>
                ...
                mfgBody.style.opacity = '1';
                mfgImg.style.opacity = '1';
            }, 200);
        };

        mfgTabs.forEach((tab, index) => {
            tab.addEventListener('click', () => {
                updateMfgContent(index);
            });
        });

        if (mfgMPrev && mfgMNext) {
            mfgMPrev.addEventListener('click', () => {
                let newIndex = (currentIndex - 1 + tabKeys.length) % tabKeys.length;
                updateMfgContent(newIndex);
            });

            mfgMNext.addEventListener('click', () => {
                let newIndex = (currentIndex + 1) % tabKeys.length;
                updateMfgContent(newIndex);
            });
        }
    }
    ...
    if (brochureModal && downloadBtn && closeBtn) {
        ...
        if (brochureForm) {
            brochureForm.addEventListener('submit', (e) => {
                e.preventDefault();
                alert('Thank you! Your brochure is being prepared and will be sent to your email shortly.');
                closeModalFunc();
                brochureForm.reset();
            });
        }
    ...
    if (callbackModal && closeCallbackBtn) {
        ...
        if (callbackForm) {
            callbackForm.addEventListener('submit', (e) => {
                e.preventDefault();
                alert('Thank you! We have received your request and will call you back shortly.');
                closeCallbackFunc();
                callbackForm.reset();
            });
        }
